...
安装脚本会下载最新 GitHub Release，校验 `checksums.txt`，如果提供 `FLOWGUARD_MINISIGN_PUBKEY` 还会验证 `checksums.txt.minisig`，然后安装 `flowguard` 二进制、安装依赖并启动安装向导。
即使通过 `curl | sudo sh` 管道运行，安装向导也会从终端读取输入。
安装向导里的裸数字会按默认单位理解：流量默认 `GB`，限速默认 `mbit`。
内置防抖机制：例如 85% 触发轻度限速，但要降到默认 80% 以下才解除，避免在边界反复限速/解限。

> 安全提示：`curl | sudo sh` 适合全新自管 VPS 的快速安装。生产环境建议先下载并审阅脚本，或固定 `FLOWGUARD_VERSION=vX.Y.Z` 后安装；使用 `FLOWGUARD_BASE_URL` 镜像时，请只使用你信任的 HTTPS 发布源。安装脚本会拒绝非 HTTPS 下载地址（localhost 测试镜像除外）。
...
| 命令 | 用途 |
| --- | --- |
| `flowguard install` | 安装依赖、生成配置并安装 systemd 服务 |
...
| `flowguard modify --language zh|en` | 切换后续命令和通知输出语言 |
...
| `flowguard upgrade --minisign-pubkey RW...` | 验证 `checksums.txt.minisig` 后再升级 |
| `flowguard upgrade --no-restart` | 只替换二进制，不自动重启服务 |
| `flowguard test-notify` | 发送 Telegram 测试通知 |
| `flowguard check-once` | 运行一次检查，适合调试或 cron |
| `flowguard limit` | 按配置对所有网卡应用强限速 |
| `flowguard unlimit` | 移除 FlowGuard 自己创建的限速 |
...
| `flowguard config-example` | 输出示例配置 JSON |
...
- 如果当前 vnStat 账期总量低于 FlowGuard 安装基线，FlowGuard 会报错并停止本次决策，避免在 vnStat 数据库被重置后低估用量。
...
- 配置文件和 `status --json` 都包含 `schema_version`，便于后续脚本判断结构版本。
...
> 提示：命令行参数可能出现在 shell 历史或短时间出现在进程列表里。更谨慎的做法是直接运行 `sudo flowguard install`，在交互向导中输入 Telegram 信息。
...
如果要安装到自定义目录，安装脚本会把 `FLOWGUARD_INSTALL_DIR` 传给内置安装向导和 systemd unit：
...
> 自定义路径请使用持久目录，例如 `/opt/flowguard/bin`。不要把二进制、配置或状态路径放在 `/tmp` 下；systemd 服务启用隔离后无法可靠使用 `/tmp` 路径，FlowGuard 会拒绝这类 systemd 安装。使用自定义安装目录后，后续升级也请继续指定相同目录，例如 `flowguard upgrade --install-dir /opt/flowguard/bin`。
...
- 配置修改会生成 root-only 备份，并最多保留最近 10 份；如果启用了 Telegram，这些备份同样包含 bot token，公开日志或工单前请先脱敏。
